...
const Branchs = React.memo((props) => {
    ...
    return (
        <App checkPagination={checkPagination} searchShow={true} pageName='Объекты'>
            <Head>
                ...
                <meta property='og:url' content={`${urlMain}/branchs`} />
                <link rel='canonical' href={`${urlMain}/branchs`}/>
            </Head>
            <div className='count'>
                {`Всего: ${count}`}
            </div>
            <div className={classes.page}>
                {list?list.map((element)=>
                    <LazyLoad scrollContainer={'.App-body'} key={element._id} height={height} offset={[height, 0]} debounce={0} once={true}  placeholder={<CardLegalObjectPlaceholder height={height}/>}>
                        <CardLegalObject link='branchs' element={element}/>
                    </LazyLoad>
                ):null}
            </div>
            {
...
Branchs.getInitialProps = async function(ctx) {
...
export default connect(mapStateToProps)(Branchs);
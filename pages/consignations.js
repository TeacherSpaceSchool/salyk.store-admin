...
const Consignations = React.memo((props) => {
    ...
    return (
        <App checkPagination={checkPagination} searchShow={true} pageName='Кредиты'>
            <Head>
                ...
                <meta property='og:url' content={`${urlMain}/consignations`} />
                <link rel='canonical' href={`${urlMain}/consignations`}/>
            </Head>
            <div className='count'>
                {`Всего: ${count}`}
            </div>
            <div className={classes.page}>
                {list?list.map((element)=>
                    <LazyLoad scrollContainer={'.App-body'} key={element._id} height={height} offset={[height, 0]} debounce={0} once={true}  placeholder={<CardLegalObjectPlaceholder height={height}/>}>
                        <CardLegalObject link='consignations' element={element}/>
                    </LazyLoad>
                ):null}
            </div>
        </App>
...
Consignations.getInitialProps = async function(ctx) {
...
export default connect(mapStateToProps)(Consignations);
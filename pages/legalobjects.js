...
const LegalObject = React.memo((props) => {
    ...
    return (
        <App checkPagination={checkPagination} searchShow={true} pageName='Налогоплательщики'>
            <Head>
                <title>Налогоплательщики</title>
                ...
                <meta property='og:title' content='Налогоплательщики' />
                ...
                <meta property='og:url' content={`${urlMain}/legalObjects`} />
                <link rel='canonical' href={`${urlMain}/legalObjects`}/>
            </Head>
            <div className='count'>
                {`Всего: ${count}`}
            </div>
            <div className={classes.page}>
                {list?list.map((element, idx)=>
                    <LazyLoad scrollContainer={'.App-body'} key={element._id} height={height} offset={[height, 0]} debounce={0} once={true}  placeholder={<CardLegalObjectPlaceholder height={height}/>}>
                        <CardLegalObject link='legalobject' list={list} key={element._id} setList={setList} element={element} idx={idx}/>
                    </LazyLoad>
                ):null}
            </div>
            {
                profile.add?
                    <Link href='/legalobject/[id]' as={`/legalobject/new`}>
...
LegalObject.getInitialProps = async function(ctx) {
    ...
    if(!['admin', 'superadmin', 'оператор'].includes(ctx.store.getState().user.profile.role))
        if(ctx.res) {
...
export default connect(mapStateToProps)(LegalObject);
...
const WithdrawHistorys = React.memo((props) => {
    ...
    return (
        <App checkPagination={checkPagination} searchShow={true} pageName='Изъятие средств'>
            <Head>
                ...
                <meta property='og:url' content={`${urlMain}/withdrawhistorys`} />
                <link rel='canonical' href={`${urlMain}/withdrawhistorys`}/>
            </Head>
            <div className='count'>
                {`Всего: ${count}`}
            </div>
            <div className={classes.page}>
                {list?list.map((element)=>
                    <LazyLoad scrollContainer={'.App-body'} key={element._id} height={height} offset={[height, 0]} debounce={0} once={true}  placeholder={<CardLegalObjectPlaceholder height={height}/>}>
                        <CardLegalObject link='withdrawhistorys' element={element}/>
                    </LazyLoad>
                ):null}
            </div>
        </App>

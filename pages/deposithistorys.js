...
const DepositHistorys = React.memo((props) => {
    ...
    return (
        <App checkPagination={checkPagination} searchShow={true} pageName='Внесение средств'>
            <Head>
                ...
                <meta property='og:url' content={`${urlMain}/deposithistorys`} />
                <link rel='canonical' href={`${urlMain}/deposithistorys`}/>
            </Head>
            <div className='count'>
                {`Всего: ${count}`}
            </div>
            <div className={classes.page}>
                {list?list.map((element)=>
                    <LazyLoad scrollContainer={'.App-body'} key={element._id} height={height} offset={[height, 0]} debounce={0} once={true}  placeholder={<CardLegalObjectPlaceholder height={height}/>}>
                        <CardLegalObject link='deposithistorys' element={element}/>
                    </LazyLoad>
                ):null}
            </div>
        </App>
...
DepositHistorys.getInitialProps = async function(ctx) {

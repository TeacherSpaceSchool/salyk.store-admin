...
const Reports = React.memo((props) => {
    ...
    return (
        <App checkPagination={checkPagination} searchShow={true} pageName={`${router.query.type}-Отчет`}>
            <Head>
                ...
                <meta property='og:url' content={`${urlMain}/reports`} />
                <link rel='canonical' href={`${urlMain}/reports`}/>
            </Head>
            <div className='count'>
                {`Всего: ${count}`}
            </div>
            <div className={classes.page}>
                {list?list.map((element)=>
                    <LazyLoad scrollContainer={'.App-body'} key={element._id} height={height} offset={[height, 0]} debounce={0} once={true}  placeholder={<CardLegalObjectPlaceholder height={height}/>}>
                        <CardLegalObject link='reports' element={element} query={{type: router.query.type}}/>
                    </LazyLoad>
                ):null}
            </div>
        </App>

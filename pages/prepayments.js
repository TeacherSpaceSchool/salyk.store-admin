...
const Prepayments = React.memo((props) => {
    ...
    return (
        <App checkPagination={checkPagination} searchShow={true} pageName='Аванс'>
            <Head>
                ...
                <meta property='og:url' content={`${urlMain}/prepayments`} />
                <link rel='canonical' href={`${urlMain}/prepayments`}/>
            </Head>
            <div className='count'>
                {`Всего: ${count}`}
            </div>
            <div className={classes.page}>
                {list?list.map((element)=>
                    <LazyLoad scrollContainer={'.App-body'} key={element._id} height={height} offset={[height, 0]} debounce={0} once={true}  placeholder={<CardLegalObjectPlaceholder height={height}/>}>
                        <CardLegalObject link='prepayments' element={element}/>
                    </LazyLoad>
                ):null}
            </div>
        </App>
...
Prepayments.getInitialProps = async function(ctx) {
...
export default connect(mapStateToProps)(Prepayments);